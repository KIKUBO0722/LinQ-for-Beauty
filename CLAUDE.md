...
## デザイン規約 (LP / 管理画面 / リッチメニュー 全 UI)

- **UI を生成・改修するときは `@DESIGN.md` を厳守すること**。配色・タイポ・余白・コンポーネント・Do/Don't はこのファイルが単一の真実源。
- 視覚的参考は `docs/design-references/01-hirokei-fullpage.png` を必ず確認してから出力する。
- 迷ったら **HIROKEI トーン** (ピンク→パープルグラデの大型英字 Hero + 円形ブロック + Venn 図 + ピル型 CTA + 広い余白 + 温度感のある人物写真) を優先。
- 「グラデは 2 箇所以下」「LINE グリーンは機能色のみ」「数値で語る」「押し売りフレーズ禁止」「余白は HIROKEI レベル」は最重要原則。
